...
% Version 1.0f, drop \errorstopmode from \loggingoutput
...
% so they are shown with teh same csname in luatex
...
function latexDefineFont(n,s,i)
  ...
  for ii,vv in font.each() do
    if (n == vv.name) then
      if(s == vv.size or (s == -1000 and vv.size == vv.designsize)) then
        f=ii
% debugging
%      else
...
if (luatexbase==nil) then
  ...
  % may need to test if this is already set and do something else if so.
