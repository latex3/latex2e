...
the removal or renaming of most of the primitve commands introduced by
...
\verb|\CheckSum| commaand then, as before, the number is checked,
...
support for non breaking hyphens and more dashes. \NOTDONE

\section{Uptdates in Tools}

The \package{varioref} package has been updated with improved
documentation of multi-linual support, and avoiding errors in
some cases with \verb|\reftextfaraway|.

The \package{tabularx} package's handling of \verb|\endtabularx| in
envionment definitions has been fixed to again match its documentation.
...
with the American Mathematical Society retaining the maintenance of
\textsf{amscls}.
...
incompatible with previous version of \package{amsmath}. It also
improves \verb|\dots| handling so that \verb|\long| macros are
correctly handled (so that  for example \verb|\dots \Rightarrow| now
uses centred dots) and of commands expanding to character tokens so
that \verb|\times \dots \times| will use centred dots wuth
