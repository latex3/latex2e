...
\subsection{Names of \hologo{LuaTeX} primitve commands}
...
produced some awqward command names such as \verb|\luatexluafunction|.
...
Note the compatibilty layer offered by the \package{luatexbase} package
...
In this release a new implementation\footnote{not yet(?) in svn} of
...
out. This allows an extra 18 (or in \hologo{LuaTeX}, 64 thousand)
...
these letters (U+0218\,--\,U+021b) and also
to support U+00A0 (\verb|\nobreakspace|) and U+00AD
(\verb|\-|). 

The error message given on undefined UTF-8 input characters has been
improved to give the Unicode slot number
...
\verb|LaTeX2e <2015/11/01> pre-release-1|}\\
Internally this is identified as a patch release with a negative patch
level.

\section{Updates in tools}

The \package{multicol} package has been updated to fix the interation
