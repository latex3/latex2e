...
which required small adjustment in the  \hologo{LuaTeX} setup.
...
need to add aliases for the old \hologo{pdfTeX}. A new
\package{luapdftexalias} has been contributed to CTAN (not part of the
...
The \package{trig} has been updated so that the pre-computed values
...
\hologo{LuaTeX} from version 0.87 no longer supports the
...
defines \verb|\write18| in  \hologo{LuaTeX} so that it does access
