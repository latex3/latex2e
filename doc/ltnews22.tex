...
\subsubsection{The \cs{IncludeInRelease} command}
...
The new concept provides full backward and forward compatibilty for
...
However, this is not necessarily true for packages in the \LaTeX{}
universe. Only if a package makes use of the \cs{IncludeInRelease}
functionality it will adjust its version to that mathcing the
requested \LaTeX{} release date. Initially this will only be true for
a few selected package an in all honesty it may not make for packages
to have their own release cycle and methods.

Thus, to regenerate a document with 100\% it will still be necessary
...
help identifying what release tree to use to achive perfect accuracy.
...
%FMi: this is true for anything we fix or add ... so why mention it here?
...
\hologo{LuaTeX} and \hologo{XeTeX} needed to over-ride the settings specified
in the \LaTeX\ kernel files.

Finally a modified definition of \verb|\typein| is used with \hologo{LuaTeX}, to
avoid the need for the \hologo{LuaTeX} configuration to patch the format.
