...
This document describes changes that have been made to the \LaTeX\ format.
...
This document details all the changes to documented functiosn since teh 2015/05/01 \LaTeX\ release.
...
The |\includeInRelease| command described in detail in the
\Lpack{latexrelease} package documentation has been added to control
the use or not of any changes to the \LaTeX\ format.

\subsection{New Allocation commands}
...
In addtion, allocation commands such as |\newcount| will allocate
extended e-\TeX registers once the original 256 registers have been used.
This is similar to functionality previously available in the \Lpack{etex}
package, but is now integrated into the core and extended for Unicode based
systems.
...
pagebreaker. (Unlike say the \Lpack{multicol} package, where all
