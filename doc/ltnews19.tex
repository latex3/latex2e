...
release and introduces no new features.
A~number of small changes have
been made to correct minor bugs in the kernel and improve various
aspects of some of the \texttt{tools} packages; we have also
slightly extended the Unicode support.
...
be distributed yearly with \TeX\ Live.
Having said this, very few bug fixes to the kernel are needed nowadays.

Remember that we cannot change any of the
...
New features cannot be added, because any new documents using
...
means for the development of a wide range of extensions.
...
integration of the test system for Babel is still not complete.


\section{From \LaTeXe\ to \LaTeX3}

Whilst our work on \LaTeXe\ will be almost exclusively maintenance,
the development work of the \LaTeX3 project is
seeing new life. Our goal here is to provide a transition from the
\LaTeXe\ document processing model to one with a more flexible
intrastructure. Work is continuing on refining and expanding the
the \textsf{expl3} programming language whilst
using it for document design in the \textsf{xpackages}.
Future announcements about \LaTeX3 will be available via the \LaTeX\ Project
