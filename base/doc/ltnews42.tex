...
A font series value like \texttt{sbc} contains both the weight (here
\texttt{sb}, i.e.\ ``semi-bold'') and the width (here \texttt{c},
i.e.\ ``condensed'') of the font. If you want to reset only one of the two to
``medium'', you can use \verb+\fontseries{m?}+ or \verb+\fontseries{?m}+:
The former switches ``sbc'' to ``c'', the latter switches ``sbc'' to ``sb''.
However, if the resulting series does not exist, you got strange warnings,
e.g.:
\begin{verbatim}
LaTeX Font Warning:
...
using `OT1/cmss/m/n' instead on input line 7.
\end{verbatim}
The source file was also tidied up a little on this occasion.
