...
\subsection[Programming support for meta font families]
           {Programming support for \LaTeX's meta font families}
...
addition, other font families can be used by the user ot in a document
class by explicitly loading them through
\cs{fontfamily}\texttt\{\meta{name}\texttt\}\cs{selectfont}.

In some cases it is helpful to know which of the three meta families
...
none of the meta families is currently used).

As a small application of this, the \LaTeX{} kernel now also contains
\cs{@reinitializemetafamily}. If the current meta family is
