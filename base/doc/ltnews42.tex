...
Currently it is difficult for document authors to change the tag names as the tagging support code uses either a fix name or some ad-hoc internal variable. We therefore added three commands that offer an interface to declare, use and reassign \emph{symbolic structure names}. \verb+\NewStructureName+ takes one argument and declares a \emph{symbolic structure name}. The expandable command \verb+\UseStructureName+ takes one argument and allows to use name in a \cs{tagstructbegin} command. \verb+\AssignStructureRole+ allows to assign the symbolic structure name a role.
