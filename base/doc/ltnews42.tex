...
and use \texttt{text} (rollmapped to \texttt{P}) only for (portions of) 
...
code into sockets. The \enquote{big paragraphs} can now be disabled by assigning to 
these socket the \texttt{noop} plug:
\begin{verbatim}
...
The first two are hooks with one argument and take as argument the value of the key. The hooks are processed even if tagging is not activated.
