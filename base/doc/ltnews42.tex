...
can reliably reliably assume that such macros are not \cs{long}
regardless of whether defined by \cs{renewcommand} or \cs{def}.

There is a small chance that this is a breaking change for some
package code (though we don't know about any case), i.e., if the code
was deliberately checking against \cs{long}\cs{def} only\Dash in that
case the test now needs to be done against the definition without
...
does not exist, you got strange warnings, e.g.:
\begin{verbatim}
LaTeX Font Warning:
...
that controls whether a \cs{par} token is added if \TeX\ is in horizontal mode
at the end of \cs{vbox} and similar contexts. This gives more control
