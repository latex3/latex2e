...
To model such \enquote{big} paragraphs \LaTeX{} uses a structure named \struct{text-unit} and use \struct{text} (rollmapped to \struct{P}) only for (portions of) the actual paragraph text\LaTeX{}. 

This is semantically clear and allows processors who care to identify
the complete paragraphs by looking for \struct{text-unit} tags. But we got also request for an option to disable the tagging of the \enquote{big} paragraphs, so with this release we put the relevant tagging code into sockets. The \enquote{big} paragraphs can now be disabled by assigning to these socket the \texttt{noop} plug:
\begin{verbatim}
\AssignTaggingSocketPlug{para/begin/main}{noop}
\AssignTaggingSocketPlug{para/end/main}{noop}
\end{verbatim}

\subsection{Hooks for \cs{includegraphics} keys}
The three keys definitions \texttt{alt}, \texttt{actualtext} and \texttt{artifact} used by \cs{includegraphics} contain now hooks, named \texttt{Gin/alt},
\texttt{Gin/actualtext}, and \texttt{Gin/artifact}. The first two are hooks with one argument and take as argument the value of the key. The hooks are processed even if tagging is not activated.
With them, it is for example possible to store the alternative text:
\begin{verbatim}
  \AddToHookWithArguments{Gin/alt}{\gdef\myalttext{#1}}
