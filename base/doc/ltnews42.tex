...
now it can be used in production workflows\Dash and already is by some users\Dash as long as one
...
example, \pkg{hyperref} is partially incompatible as the form fields
...
strange warnings, but this should affect only very few fonts. The
...
Previously attempts to add hooks to commands would fail if the original definition
had used  \verb|~| in an \verb|\ExplSyntaxOn| context.
%
\githubissue{1099}
...
\pagebreak
while others are very widely used in
