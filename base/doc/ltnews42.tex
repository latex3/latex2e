...
To model such \enquote{big} paragraphs \LaTeX{} uses a structure named \texttt{text-unit} and use \texttt{text} (rollmapped to \texttt{P}) only for (portions of) the actual paragraph text\LaTeX{}. 

This is semantically clear and allows processors who care to identify
the complete paragraphs by looking for \texttt{text-unit} tags. But we got also request for an option to disable the tagging of the \enquote{big} paragraphs, so with this release we put the relevant tagging code into sockets. The \enquote{big} paragraphs can now be disabled by assigning to these socket the \texttt{noop} plug:
\begin{verbatim}
