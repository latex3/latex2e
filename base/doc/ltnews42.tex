...
\enquote{infinite shinkage error}. By now the engines got a new
primitive \cs{ignoreprimitiveerror} which can be used to turn this
error into a warning, for example, when you do only trial splitting of
...
0 (unless there are real errors) so in workflows that want to test if
a \TeX{} run ended without errors, you don't get a bogus result
because we do not have an error any longer.
%
\githubissue{1750}
