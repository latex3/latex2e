...
up where user want, for example, to apply a color change to all mathematics. With
...
\subsection{Using \cs{DocumaentMetadata} more than once}

Until now it was possible to use \cs{DocumentMetadata} more than once,
the second use executed only some keys. This was changed, similar to
\cs{documentclass} the \cs{DocumentMetadata} declaration will now
error if used more than once.
...
contrived example, the problem is real and shows up in documents and
for documents using \cs{DocumentMetadata} it has been corrected.
%
\taggingissue{1328}


\subsection{Making the design of the \env{decription} environment more flexible}
...
galley and in footnotes, etc. After some experimentation we extended
...
The tagging code extends the \env{picture} environment to take a
...
commands that save and restore the last skip to prevent that these
anchors disturb spacing commands like \cs{addvspace}. We now provide
...
\subsection{Container for document properties and metadata}
We now provide a container for document properties and metadata. 
Value can be added to this container with \cs{AddToDocumentProperties}, a value
get be queried expandably with \cs{GetDocumentProperty}:
\begin{verbatim}
\AddToDocumentProperties[mypackage]{key}{value}
\GetDocumentProperty{mypackage/key}
\end{verbatim}
Packages adding properties should use a unique label (by default the package name is used). The label \texttt{document} is reserved. More documentation can be found in source2e in the section ltmeta. 

When \cs{DocumentMetadata} was used or the PDF management was loaded this
container already existed for some time, e.g. , the new \texttt{language} key mentioned
above uses it to store its value. We now provide it generally to allow 
class and packages to add and query values without having to worry if 
\cs{DocumentMetadata} has been used or not. 
