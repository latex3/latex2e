...
continues. The implementation of black structures, such as theorems, and
...
addressed a long-standing technical wrinkle: using math mode simply to align
material vertically. This \enquote{fake math} is a tagging issue but also shows
up where user want for example to apply a color change to al mathematics. With
this release, the use of math mode is essentially restricted to \enqupote{true
math}.

Working on templates has prompted us to revise some concepts that have been
added to the kernel in recent years, to align the code more closely with the
most natural pathways for users.

Wok of course is not restricted to tagging. There are further improvements to
...
\subsection{Support \cs{newtheoremstyle}}
