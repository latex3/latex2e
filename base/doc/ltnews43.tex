...
document, currently called \texttt{latex-cmds.pdf}. The aim over time will
be to include all of new features here. We also hope to collect up existing
documentation in the longer-standing sources (going back to before \LaTeXe{}!).
...
In 2021 when compiling with \pdfTeX{} additional information
(from the file \file{glyphtounicode.tex}) was added automatically
to the PDF file in order to improve copying from, and searching in,
text for, e.g., common ligatures.

We now extended this support and load additionally a file \file{glyphtounicode-cmex.tex}
which improves copying of mathematical symbols.

As starting with version 1.24 \luatex{} will support an extended syntax of \cs{pdfglyphtounicode} that makes it possible to improve copy\,\&\,paste of symbol fonts we load and activate the files also with this engine if PDF mode is active. Other engines do not support \cs{pdfglyphtounicode} and \cs{\cs{pdfgentounicode}. Here we define the commands as noop commands to allow package for symbol fonts to use the commands without testing the engines first. 
%
\githubissue{2007}
