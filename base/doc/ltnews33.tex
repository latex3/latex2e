...
\subsection{\class{ltxdoc} us now using new beta version of new \pkg{doc}}

We are now internally using the \pkg{doc} package in version 3 and
will make that the default version in the Fall release.  For now it
exists as \pkg{doc-v3beta} and is used internally to format the
\LaTeX{} sources.  We also now distribute the \texttt{ltxdco.cfg}
configuration file we use to format them.
