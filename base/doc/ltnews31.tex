...
In TeXLive 2020 the Lua\LaTeX{} format will use the new luahbtex engine which is luatex with an embedded HarfBuzz library.
HarfBuzz can be used by setting a suitable renderer in the font declaration, an interface is provided by \pkg{fontspec}. These additional font renderer will greatly improve the shaping of various scripts which are currently handled correctly only by \XeTeX{}. To simplify the testing of the new engine binaries have been added to MiKTeX and TeXLive 2019 and both have changed the Lua\LaTeX-dev format to use it.
