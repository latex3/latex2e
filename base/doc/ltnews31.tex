...
The \LaTeX{} programming layer, \pkg{expl3}, has over the past decade moved
from being largely experimental to broadly stable. It is now used in a
significant number of third-party packages, most notably \pkg{fontspec}, which
is loaded in most \LaTeX{} documents compiled using \XeTeX{} and \LuaTeX{}.

The \pkg{expl3} layer contains a non-trivial number of macros, and when used
with the \XeTeX{} and \LuaTeX{} engines loads a large body of Unicode data.
This means that even on a fast computer, there is a non-trivial load time for
using \pkg{expl3}.
...
at the user level, and loading of \pkg{expl3} by programmers is similarly
unaffected: they may use
\begin{verbatim}
\RequirePackage{expl3}
\end{verbatim}
or similar as normal.
