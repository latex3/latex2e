...
%% For "ipadtex", noting that Chris needs to get himself a full (auto-maintained!) 
%%   development system! (not activated)
...
\def \githubissue #1{\\github issue #1}

% simple solution right now (just link to the first issue if there are more)
\def\getfirstgithubissue#1 #2\relax{#1}
  
\def \taggingissue #1{\\tagging-project issue #1}
...
in addition to this tagging-focussed work, we have also made advances in these areas:
 a new argument type in 
\pkg{ltcmd} (\pkg{xparse}) 
%% why are these packages named here?  where is the new arg type? 
which will make working with verbatim
content a \emph{lot} easier; further improvements to case changing; and, of
course, many bug fixes.
...
\end{old}  
First, here is a description of the new hooks:
%. OR "some of" or "five of" these new hooks:  ??

%% needs good intro !! Two hooks: %% ?? 
%% looks like 5 ! 
\begin{description}
...
\item[\hook{build/page/before}, \hook{build/page/after}]
   ...
   %. ?? see below re "mirrored hooks". 
   Technically, they are executed at the start and the end, respectively, of the
   internal \LaTeXe{} \cs{@outputpage} command.  Currently, a number of packages change this command by adding code in exactly
...
\item[\hook{build/column/before}, \hook{build/column/after}]
   ...
   implemented as mirrored hooks.
 %% DO WE KNOW what "mirrored hooks" are?
 %% FMi: yes we do, we can't reexplain everything each time. This is not a new concept
   Technically, they are executed at the start and the end, respectively, of the
   internal \LaTeXe{} \cs{@makecol} command.  A
...
support tagging. 
%% NEEDED?
%% Only TWO feature below, I think! Something missing, perhaps?
\marginpar{FIX}
Three of these sockets are of interest for use in
class files and also in the document preamble. 
%%  ??  so here we ?? 
%% . . . ??
%% ?? Only one of these 3?? has a very long explanation, 
%%    one is a small extra addition, one is non-existent !! 
...
%z why "predefined" 
%% You might need to define this plug too!
...
\end{old}
The predeclared\marginpar{FMi: prefer my version}
%. declared vs. defined??  OR  available 
plugs are as follows:
%%  ?? 

We have provided the following (predeclared) plugs for this socket:
%% How to prevent a column break here? 
\begin{description}
...
mark mechanism for \LaTeX{}, one that supports 
%% arbitrarily many 
any number of truly
independent mark classes. This mechanism also offers the ability to query the state 
%. OF WHAT? 
at the top
of the page, something that wasn't previously available in at all.
...
implemented the same public interfaces using the new concepts.
Thus the old commands 
(\cs{markboth}, \cs{markright}, \cs{leftmark} and \cs{rightmark})
remain supported, but internally these commands all use \cs{InsertMark}, etc.  
...
flexibility, 
%% Not sure I follow much of this next bit! 
...
%% Need to explain, at least by a ref, what "tagging" actually is!

A lot of the tagging support in packages is handled through the socket-and-plug 
%% hyphenation is used to indicate that this a single concept, 
%% not two things: "socket" and "plug"
...
Sockets and plugs offer an interface\fmi{think this is a wrong rewrite} 
...
%% ?? 
%% can be changed from the outside so as to, for example, alter the processing.
%% from the outside ??  of what?

For the tagging support, a specialised set of sockets is available: 
%FMi: these sockets are activated
...
%% The logic here is somewhat more complex!
%% Need to explain why mml gets involved! 
%%  and maybe also what an  "accessible PDF" is, in context? 
Creating a PDF 2.0 version is considered essential for any document that has 
substantial mathematical content.  This is because only this version supports 
the straightforward use of tags from the 
MathML namespace. When \cs{DocumentMetadata} is used, \LaTeX{}
...
tagging variants: 
%% But what is a "tagging variant"? Maybe, possibilities, or methods ??
%% And what is a "replacement for symbol", ?? 
...
%% what code?  supports how? 
graphics produced using the 
\pkg{tikz} packages and \enquote{todo notes} from the \pkg{todonotes} package.
%% are todo notes at all related to graphics?? 
...
simplify the setup 
%% setup of what??
a new key \texttt{math/setup} can be used in the
...
\marginpar{Fix ``key in the preamble''}
...
%% officially supported meaning what?  
...
It is important to understand that these accommodations do not apply 
...
\begin{old}
The addition of these \cs{providecommand} lines to
classes and packages ensures that they will work with older \LaTeX{}
kernels.
\end{old}
The consistent use of these two commands in\marginpar{new is wrong} 
...
In \LuaTeX{} the situation is much better because there the structures
can be added later when the formula processing has already finished.
%
\taggingissue{762}
...
difficult to attach \TeX{} code for tagging to such display math
formulas whilst ensuring that such code always stays on the same page
as the formula, i.e., code has to be attached after the the end of the
display, but before \TeX{} places a \cs{postdisplaypenalty} onto the
...
a user to add an explicit \cs{postdisplaypenalty} or a
\cs{belowdisplayskip} setting inside the formula. We have now slightly
altered our algorithm making such user adjustments possible again.
%
\taggingissue{809}
...
Counter commands such as \cs{alph}, \cs{stepcounter}, may now have the
argument \texttt{*} to denote the current counter (as used by
\cs{label}).  This is compatible with the package \pkg{enumitem} use
of \verb|\alph*| in item labels but is generally available.  Not all
commands accept \verb|*|, for example \verb|\counterwithin| and
\verb|\counterwithout| require counter names as before.
%
\githubissue{1632}

\subsection{Collecting environment bodies verbatim}

The mechanisms in \pkg{ltcmd} (\enquote{\pkg{xparse}}) offer a
...
argument is useful. It is also possible in \pkg{ltcmd} to define
arguments which grab their content verbatim, another specialist
argument form. To date, however, it was not possible to combine these
two ideas.
...
We have looked again at how to implement \cs{MakeTitlecase} to be as
...
In \LaTeX{} News~38, we described the extension of \cs{verb}, etc., to
cover the tab character as equivalent to a space. We have now added
tabs to the standard list of characters covered by
\cs{dospecials}. This allows them to be used in for example a
\texttt{v}~specification document command without additional steps.

\subsection{Refinement of \texttt{v}~specification category codes}

Work on verbatim argument handling has highlighted that storing all
...
\cs{DeclareTextSymbol}, \cs{DeclareTextCommand}, and friends log their
changes, but in contrast to their math counterparts they never
did. This has now finally changed.
%
\githubissue{1242}
...
information is written to the log and terminal. Users may wish to make
this into an error, in which case they should set the value to~5
(not~3); this works in all engines.

\githubissue{1687}

\subsection{Always use the extended pool}
...
command \cs{FOO} isn't defined later, e.g., in a package, it now
raises an error if it is used in the document.
%
\githubissue{1591}
...
lead to spurious warnings. This has now been corrected: global options
are seen exactly once per package by the keyval-based option handling
system.
%
\githubissue{1619}
...
it was reusing the value in that internal variable as the starting
...
\cs{DeclareEncodingSubset} declaration into each
\texttt{ts1\meta{family}.fd} file so that it is tied to the font
definition and available if a font family is explicitly selected
through \cs{fontfamily}\texttt{\{\meta{name}\}} instead of using some
font support package. Unfortunately, doing this could result in
incorrectly selected glyphs when the font encoding subset setting was
evaluated before the \texttt{.fd} file was loaded (because then subset
9 was assumed).  This has now been corrected and
...
then these settings will remain active while the output routine builds
the page. This is normally harmless, because the material used for the
...
change later on. This has been corrected and the arguments are fully
expanded when the declaration is made.
%
\githubissue{1675}
...
its argument, so \verb*=Hello\textcolor{red}{ World}=
will output Hello\textcolor{red}{ World}.  There are technical as well
as compatibility reasons for this, so the behavior will not
change. This has now been clarified in the documentation.
%
\githubissue{1474}
...
\env{gather} environment this offset was not reset correctly, with
the result that it also got applied to any following environment,
resulting in incorrect spacing in certain situations. The fix for
this, implemented in 2024/06, was not fully correct: we now do the
resetting at the start of all display math environments.
%
\githubissue{1289}
...
tagging code to provide an ActualText string and a boolean flag that
...
\cs{reftextfaceafter} produce nothing, one has to get rid of the space
that is automatically placed in front of the command. This can be done
...
Work on the L3 programming layer continues in parallel with development of the
\LaTeX{} kernel. Of note for developers is that we have integrated more code
into the main \pkg{l3kernel} bundle, and therefore into the functionality
available automatically within \LaTeX{}. Most notably, \pkg{l3benchmark}, which
provides tools for checking code performance, is now part of \pkg{l3kernel}.

We have also extended the \pkg{color} module to recognise the Oklab and Oklch
color model: thanks to Markus Kurtz for contributing this code. The Oklab color
space (\url{https://bottosson.github.io/posts/oklab}) is a perceptual color
space which is supported by CSS and so modern web browsers.

The final \enquote{highlight} here is for developers who want to create
packages that work not only with \LaTeX{} but also with other formats: generic
mode \pkg{expl3}. Supporting Con\TeX{}t, in particular when used with
LuaMeta\TeX{}, requires some work in keeping up with engine changes. After a
period in which the L3 programming layer was not working there due do
extensions to (mainly) \cs{numexpr}, work has now been completed to get this
support working again. As well as adapting to the extended \cs{numexpr} syntax,
we have also track changes in the format of \cs{meaning} and the subtle
differences between \cs{scantokens} and \cs{scantextokens}. Not \emph{everything}
works in generic mode, but we do aim to have a broadly working format-neutral
codebase.
