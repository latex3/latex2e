...
With the \LuaTeX{} engine there are now various option for accessible math described in full details in latex-lab-math.pdf. To simplify the setup a new
...
PDF 2.0 is a requirement for accessible PDF containing math as the MathML name spaces isn't supported by earlier PDF version. \LaTeX{} will there set
PDF 2.0 as default version if \cs{DocumentMetadata} is used. Other versions can be set with the \texttt{pdfversion} key.  
