...
\item[\hook{build/column/before}, \hook{build/column/after}]
 ...
 %% FMi: yes we do, we can't reexplain everything each time. This is not a new concept and discussed in earlier ltnews and elsewhere
   Technically, they are executed at the start and the end, respectively, of the
...
any number of truly
independent mark classes. This mechanism also offers the ability to query the mark status 
at the top
...
flexibility, 
%% Not sure I follow much of this next bit! %FMi better?
e.g., by being able to display not only the last right-mark
(\cs{leftmark} or \verb=\LastMark{2e-right}=) but also  or alternatively the first
...
%FMi intro yes, but we don't need to explain tagged PDF ... it is discussed since 2020 in every ltnews
...
MathML namespace.
%
In earlier PDF versions, formulas can only be made accessible by describing their content in an \texttt{alt} attribute.\marginpar{added}
%
...
%% are todo notes at all related to graphics?? %FMi: yes there are boxes(graphics) in the margin
...
\cs{tagpdfsetup} (or in \texttt{tagging-setup} as show above)
...
%% the example below is very bad as an example of typesetting !! 
%%  and its content is even worse! In fact it is "nonsense"!! 
%% Why the ... is it centred ???
%%FMi:  center is indeed problematical as it looks like c - tried flushright
\marginpar{Joseph?}
Thus, we may use
\begin{verbatim}
...
%% :-) 
%% 
%% why is this centred? it looks terrible! 
\begin{quote}
\makeatletter
\def\@verbatim{%
  \trivlist
...
\begin{verbatim}
  ...
  $y = mx + c$
\end{verbatim}
%% why does this get centred? 
%%   it looks very bad! 
%%  AND the example is weird: the "code" is in fact a math formula!!  
\end{quote}
...
In \LaTeX{} News~38, we described a change to \cs{verb}, etc., that makes
...
system.
\end{old}\marginpar{don't think the rewrite improves the text}
lead to spurious warnings. This has now been corrected so that now 
each global options is seen, by the keyval-based option handling
...
color models;
%% Or is there just the one model?
...
%% ??? why is this "other formats". should there be an "eg" or "such as" somewhere?
%% And: what is this "generic mode"?
...
%% extensions where?? and where are these extensions?
