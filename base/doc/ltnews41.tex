...
now been corrected.  The gymnastics to achieve this are rather an
\enquote{interesting} study in obfuscated \TeX{} coding\Dash this is
in \file{latex-lab-math.pdf} for the interested.
...
end of the display, but before the low-level \TeX{} adds a
...
we have to resort to complex gymnastics: we set
...
with it \enquote{todo notes} from the \pkg{todonotes} package. The
...
argument is useful. It is also possible using this mechanism to define
...
\cs{obeyedline} marker, which as standard issues a normal
...
their changes: but, in contrast to their math counterparts, they never
...
\cs{tracingnone} no longer alter \cs{tracinglostchars} but retain its
current setting.

The default value used in \LaTeX{} is set so that lost character
information is written to both the log and the terminal.  Users may
wish to make this into an error, in which case \cs{tracinglostchars}
should be set to~5 (not~3) as this works in all engines.
\githubissue{1687}


\subsection{Always use the extended pool of registers} 

As the kernel has grown, the use of registers has expanded to the
...
are that \cs{input} records which files are read and provides pre- and
post-file hooks.
%
...
which is internally available for programmers as \cs{@@input}).
%
\githubissue{514}
...
means that different formulas can make use of different alphabets, and
so the chances are much higher that the processing of a complex the
...
error message is issued. To circumvent the issue in that case, simply
...
\texttt{b} floats). The resulting space requirement is then stored in
an internal variable and compared to the space still available on the
page.  If the test fails, the algorithm tries the next area.
...
concluded that a float doesn't fit, even when it would in fact fit
there.  This has now been corrected.
%
\githubissue{1645}
...
If an \cs{arraystretch} greater than one is used, table rows are
...
to be larger than should have been. This has now been corrected.
%
\githubissue{1730}
