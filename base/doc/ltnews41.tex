...
A non-\cs{immediate} \cs{write} command, issued after the final page
has been shipped out, is never written because it is waiting for
another \cs{shipout} to happen. To ensure that such \cs{write}s, for
example to the \texttt{.aux} file, are nevertheless written, \LaTeX{}
is now changing its behavior after the final \cs{shipout} and makes
any further \cs{write} statements automatically \cs{immediate} (i.e.,
before it executes code in the \hook{enddocument/afterlastpage} hook).
%
\githubissue{1689}
