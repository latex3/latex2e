...
tagged. Instead, such definitions should make use of the kernel commands
\cs{dollardollar@begin} and \cs{dollardollar@end} to specify where the
math display starts and ends. This iss an essential requirement to
...
\end{verbatim}
and replace every usage of \verb=$$= with the appropriate start and
end command. By adding the \cs{providecommand} lines in the package or
class, this works also with older \LaTeX{} kernels.
...
\subsection{Local changes to the spacing around math displays}

Due to \TeX{}'s low-level handling of display math, it is very difficult
to attach tagging code to such display formulas whilst ensuring that such code
always stays on the same page as the formula, i.e., it has to be
attached before \TeX{} places a \cs{postdisplaypenalty} onto the
...
With our first implementation of this approach it was no longer
possible for a user to add an explicit \cs{postdisplaypenalty} or a
one-off \cs{belowdisplayskip} setting into the formula. We have now
