...
If a page break happens in the middle of an environment that sets up
special \cs{catcode} settings, e.g., a \env{verbatim} environment,
then these settings are active while the output routine builds the
...
reset, so it is also possible to overwrite \LaTeX{}'s behavior is that
