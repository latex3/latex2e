...
Up-to-now users had to activate tagging by loading modules from latex-lab with
the \texttt{testphase} key. Configuration of the tagging had to be done
with the \cs{tagpdfsetup} command. 
We now offer also keys with a less experimental naming.
...
The key loads the \pkg{tagpdf} package and all modules from the \texttt{testphase=latest}
...
The \texttt{tagging-setup} key allows to configure the tagging. It too
loads all modules from the \texttt{testphase=latest}
set\footnote{and so activates tagging by default, use \texttt{tagging=off} to avoid this.}. It accepts as value all 
...
With this new keys are standard setup could look like this:
\begin{verbatim}
\DocumentMetadata{
