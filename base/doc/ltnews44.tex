...
paragraph with a \texttt{semantic-para} tag and inside individual text
blocks with their own tags, i.e., in the above example the text
\texttt{Don ... writes:} and the text \texttt{This is ...!} each with
a \texttt{text-block} tag and the quote with a \texttt{quote}
...
a \texttt{<P>} and thus effectively claiming that they are individual
paragraphs while in reality they form together a single pargraph.}

Initially, we called these paragraph-related tags \texttt{text-unit}
(now \texttt{semantic-para}) and \texttt{text} (now
\texttt{text-block}) but the original names were difficult to
understand, so we changed them now.

We also introduced the \texttt{text-fragment} tag for use inside
\texttt{text-block}s if there is a need for a sub-structure.



\subsection{Revisiting the para-handling logic for block environment}
...
therefore added a new boolean template key \texttt{standalone} which
if set to \texttt{true} causes the environment to surround itself with
...
\subsection{Support \texttt{force-unnumbered} key in heading templates}
...
\subsection{Support \texttt{placement=ragged} in heading templates}

The key{placement} key was augmented to support the value
\texttt{ragged} in addition to \texttt{page}, \texttt{top}, and
\texttt{normal}. This works like \texttt{normal} (i.e., the heading
...
not spread out to fill the page in \cls{book} or \cls{report}
...
\key{placement} key to \texttt{ragged} or for individual headings in
...
\subsection{Provide an \texttt{order} key mechanism for templates}
...
\subsection{Replaced \texttt{legacy-code} key in blockenv template}

The blockenv template had a \texttt{legacy-code} key, used to support
some legacy setup for the generic \env{list} environment and for the
\env{verbatim} extension offered by the \texttt{ltugboat} class where
...
\texttt{early-legacy-code} which is evaluted near the start of the
template, possibly still in horizontal mode (used by \env{list}) and
\texttt{late-legacy-code} which is evaluted after returning to
