...
only limited flexibility, modeling layouts closely related to those from
the standard \LaTeX classes. As a consequence, they used keys such as
\key{number-title-sep} to define the space between heading number and
title on the assumption that both are placed exactly in that order.
...
then you can (for now) continue to use them simply by changing the
template names in your instance declarations from \meta{name} to
...
can be selected through options to the package. The layouts can
then be further customized through a serious of commands defined by
the package. Due to its age and the fact that it overwrote various
...
such variants. There is a downside to this: in several designs we end up
with a number of empty \texttt{heading-fragment} structures. While this
is allowed and doesn't do any harm (other than resulting in some
...
The \pkg{listings} package offers various verbatim-like environments and commands
for pretty-printing source code. We now created a style that is loaded automatically
after the \pkg{listings} package to make it compatible with tagging. 

There are still some open points, e.g., frames are not perfectly tagged, when escaping to
math the math is not tagged; when using braces to delimit the argument of \cs{lstinline}
there is no tagging, but for most standard uses it is now possible to use the package.
%
...
When using the \pkg{nameref} package it is possible to reference the names of theorems (as given by the optional argument) and to reference the optional argument of items of description lists. \pkg{nameref} made this possible by patching the internal commands of theorems and lists. These patches no longer worked with the new theorem and list code when \cs{DocumentMetadata} is used. This has now been repaired: Theorems and items with optional arguments will update \cs{@currentlabelname} and so allow \cs{label} to store this value for later reference. Items will also store the optional argument in \cs{@currentlabel} and so \cs{ref} will reference this value:
\begin{verbatim}
\begin{enumerate}
\item First item 
\item[1b] \label{item}
...
Due to an issue in the definition of \cs{math\string_register\string_env:nn}, adding to the \hook{env/\meta{env}/end}
hook for a math environment such as \env{equation} produced an error.
The definition of \cs{math\string_register\string_env:nn} has been fixed and made equivalent to what was previously
called \cs{math\string_register\string_halign\string_env:nn}. The latter command has been removed.
%
\taggingissue{1603}
...
To better support the declaration of template instances we now provide the
conditional \cs{IfTemplateExistsTF} (and its \texttt{T} and \texttt{F}
variants) so that it is possible to take different actions depending
on whether or not a certain template is declared.
...
The \cs{genfrac} command relies on engine functionality to place delimiters
around fractions in traditional \TeX\ engines but emulated this functionality
in \XeTeX\ and \LuaTeX. For \LuaTeX\ this emulation was broken and is not
needed and interferes with tagging, therefore it has been removed.
%
\githubissue{432}
...
We have adjusted the sources for \emph{\LaTeX{} News} (including this news!) to
enabling tagging (PDF/UA-2) automatically. This applies to the combined
newsletters and each individual file. This is part of our effort to make all
\LaTeX{} documentation accessible over time.
