...
When using the \pkg{nameref} package it is possible to reference the names of theorems (as given by the optional argument) and to reference the optional argument of items of descriptions list. \pkg{nameref} made this possible by patching the internal commands of theorems and lists. These patches no longer worked with the new theorem and list code when \cs{DocumentMetadata} is used. This has now been repaired: Theorems and items with optional argument will update \cs{@currentlabelname} and so allow \cs{label} to store this value for later reference. Items will also store the optional argument in \cs{@currentlabel} and so \cs{ref} will reference this value:
\begin{verbatim}
\begin{enumerate}
\item First item 
\item[1b] \label{item}
