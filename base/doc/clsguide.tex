...
Here is the example from \ref{subsec:newsletter}, using the key-value approach:
\begin{verbatim}
   \NeedsTeXFormat{LaTeX2e}
   \ProvidesClass{smplnews}[2022-06-01 The Simple News newsletter class]
...
\begin{verbatim}
   ...
   \LoadClass{article}
\end{verbatim}
In the first two, the \textsf{article} class will be loaded with option
|landscape| precisely when the current class is called with this option. By
contrast, in the second two it will never be called with option |landscape|
...
\subsection{Querying document command argument specifications}

There are some specialized cases in which a package author may need to query
the argument specification of a command. When commands are created using
\cs{NewDocumentCommand}, etc., this is possible using the \emph{code} level
function \cs{\detokenize{cmd_arg_spec:N}}. The function will \emph{expand} to
the argument specification if available, or \texttt{X} if this is not
available. The latter case is true for undefined commands and those created
using \cs{newcommand}, \cs{DeclareRobustCommand} or \cs{def}.
