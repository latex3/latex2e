...
environments. However, the bulk of code in a typical \texttt{.dtx} file will be 
within \env{macrocode} and \env{macrocode*} environments, and these were not
affected by adding \pkg{upquote}. We have now updated \pkg{doc} so that 
...
\subsection{Handling of end-of-lines in \texttt{+v} arguments of \cs{NewDocumentCommand} and friends}
...
release 2020-02-02~\cite{39:ltnews31}, the default type of its package messages was changed from package info (\texttt{Package textcomp Info})
