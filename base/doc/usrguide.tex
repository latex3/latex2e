...
\begin{decl}
  |\underline| \arg{text}
\end{decl}
Underlining may be used in math or text mode: the two have different semantics.
The same command may be used in both cases: the kernel will adopt the
appropriate structure.

\begin{decl}
  |\underline@text@vgap|    \\
  |\underline@text@thickness| \\
  |\underline@text@kern|
\end{decl}
The standard settings for these commands use the same placement parameters as
\TeX{} would use in math mode.\footnote{Again, this reflects the history of
these commands: previous versions were implemented using math mode internally
for placement.} The three settings are all dimension expressions, and are
respectively
\begin{itemize}
  \item The vertical offset of the underline below the bottom of the text.
  \item The thickness of the underline rule.
  \item The vertical gap below the rule.
\end{itemize}
The naming here follows the pattern established by OpenType math parameters.
