...
\begin{itemize}
  \item The vertical offset of the sub- or superscript: this is a dimension
    expression (as described for \cs{dimeval}), stored in a command so that it is evaluated at the time of use. The standard setting
    ...
    |\renewcommand*\textsuperscript@offset{0.86ex}| will set
    the vertical offset to $86\,\mathrm{\%}$ of the x-height of the current
    font.
  \item The space added after the sub- or superscript: the standard setting is
    a non-breakable space of the same size as would be used in math mode
    (\cs{scriptspace}). It may be preferable in new documents to allow a break
    here, as the space is then discarded, for example
