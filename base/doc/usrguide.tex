...
commands to updated values, both dimension expression (see \cs{dimeval}):
\begin{itemize}
  \item The vertical offset of the sub- or superscript. The standard setting
  ...
  \item The space added after the sub- or superscript: the standard setting is
    a fixed distance which is the one that would also be applied
    in math mode (\cs{scriptspace}).
\end{itemize}
