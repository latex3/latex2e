...
\subsection{Lowlevel \TeX{} registers and streams}
...
\begin{function}[]
  ...
  \end{syntax}
  A new integer (count), dimen, skip, toks register is allocated and
...
\begin{function}[]
  ...
  A new read or write stream is allocated and the csname \meta{cmd}
  refers to it.
...
\begin{variable}[]
  ...
  register (defined internally with \cs{newcount}) with the
  name \cs{c@\meta{name}}.
\end{variable}
...
me with initial sorting. Eventually though would all vanish andd get
replaced more by headings based on functionality, because the two do
