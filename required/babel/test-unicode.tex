...
% Unfortunately, mcb redefines callback.register to raise an error
% :-(, so for the following to work it must be placed between fontenc
% and luaotfload (which loads mcb). I'm investigating a workaround
% (something like interpreter.lua).
%
...
\usepackage[multienc,strings=unicode]{babel}
